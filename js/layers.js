	...
	challenges: {
		...
		31: {
			...
			completeInBulk() {
				...
				let target = player.points.div("1").max(1).log("1").root(2.5)
		...
		32: {
			...
			completeInBulk() {
				...
				let target = player.points.div("1").max(1).log("1").cbrt();
...
addLayer("q", {
	...
	upgrades: {
		...
		11: {
			...
			cost() { return player.q.time.plus(1).pow(1.2).times(1).pow(player.ma.current=="q"?this.id:1) },
		...
		12: {
			...
			cost() { return player.q.time.plus(1).pow(1.4).times(5).pow(player.ma.current=="q"?(Math.pow(this.id, this.id/10)*(this.id-10)):1) },
		...
		13: {
			...
			cost() { return player.q.time.plus(1).pow(1.8).times(7).pow(player.ma.current=="q"?(Math.pow(this.id, this.id/10)*(this.id-10)):1) },
		...
		14: {
			...
			cost() { return player.q.time.plus(1).pow(2.4).times(6).pow(player.ma.current=="q"?(this.id*6):1) },
		...
		31: {
			...
			effect() { return player.q.buyables[11].sqrt().times(696969).times(improvementEffect("q", 31)) },
...
addLayer("ma", {
	...
	masteryGoal: {
		p: new Decimal("1e1148"),
		b: new Decimal(208),
		g: new Decimal(125),
		t: new Decimal(84),
		e: new Decimal("e34690"),
		s: new Decimal(81),
		sb: new Decimal(3),
		sg: new Decimal(2),
		q: new Decimal("e48000"),
		h: new Decimal("e41600"),
		o: new Decimal(1e3),
		ss: new Decimal(2),
		m: new Decimal("1e10735"),
		ba: new Decimal("1e20750"),
		ps: new Decimal(15),
		hn: new Decimal("1e311"),
		n: new Decimal("1e37"),
		hs: new Decimal("1e51"),
		i: new Decimal(3),
	},
...
addLayer("en", {
	...
	milestones: {
		0: {
			requirementDescription: "80 Energy in one reset",
			done() { return player.en.bestOnReset.gte(80) || hasAchievement("a", 151) },
		...
		1: {
			requirementDescription: "2 Energy in one reset",
			done() { return player.en.bestOnReset.gte(2) || hasAchievement("a", 151) },
		...
		2: {
			requirementDescription: "3 Energy in one reset",
			done() { return player.en.bestOnReset.gte(3) || hasAchievement("a", 151) },
		...
		3: {
			...
			requirementDescription: "25 Total Energy & 26 Thoughts",
			done() { return (player.en.total.gte(25) && player.ne.thoughts.gte(26)) || player.en.milestones.includes(3) },
		...
		4: {
			...
			done() { return player.en.bestOnReset.gte(100) || hasAchievement("a", 151) },
